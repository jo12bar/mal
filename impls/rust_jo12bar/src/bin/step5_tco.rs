...
fn eval_def_bang(exprs: Vec<Expr>, env: Env) -> ExprResult {
    ...
    match &exprs[1] {
        Expr::Constant(sym @ Atom::Sym(..)) => {
            ...
            env_set(&env, HMK::try_from(sym.clone())?, evaluated_expr_2.clone());
            Ok(evaluated_expr_2)
        }
...
fn eval_let_star(exprs: Vec<Expr>, env: Env) -> Result<(Expr, Env), Box<dyn std::error::Error>> {
    if exprs.len() != 3 {
        return Err(Error::s(format!(
        ...
        Expr::List(expr1_vec) | Expr::Vec(expr1_vec) if expr1_vec.len() % 2 == 0 => {
            ...
            for (k, v) in expr1_vec.chunks_exact(2).map(|s| (&s[0], &s[1])) {
                ...
                match k {
                    ...
                    Expr::Constant(sym @ Atom::Sym(..)) => {
                        env_set(&let_env, HMK::try_from(sym.clone())?, evaluated_v);
                    }
